d3 = (function () {
  ...
  d3.behavior.drag = function () {
    ...
    function mousedown() {
      var target = this,
        ...
        touchId = d3.event.touches
          ? d3.event.changedTouches[0].identifier
          : null,
        offset,
        origin_ = point(),
        moved = 0;
      var w = d3
        .select(d3_window)
        .on(
          touchId != null ? 'touchmove.drag-' + touchId : 'mousemove.drag',
          dragmove
        )
        .on(
          touchId != null ? 'touchend.drag-' + touchId : 'mouseup.drag',
          dragend,
          true
        );
      if (origin) {
  ...
  d3.select = function (node) {
    var group = [
      typeof node === 'string' ? d3_select(node, d3_document) : node,
    ];
  ...
  d3.xhr = function (url, mimeType, callback) {
    var xhr = {},
      ...
      request = new (
        d3_window.XDomainRequest && /^(http(s)?:)?\/\//.test(url)
          ? XDomainRequest
          : XMLHttpRequest
      )();
    'onload' in request
  ...
  d3.format = function (specifier) {
    ...
    return function (value) {
      ...
      var length =
          basePrefix.length + value.length + (zcomma ? 0 : negative.length),
        padding =
          length < width
  ...
  d3.geo.bounds = (function () {
    ...
    return function (feature) {
      ...
      for (
        var i = 1, n = ranges.length, a = ranges[0], b, merged = [a];
        i < n;
        ++i
      ) {
        b = ranges[i];
  ...
  d3.geo.centroid = function (object) {
    ...
    if (
      ...
      Math.abs(
        (m = Math.sqrt(
          d3_geo_centroidX * d3_geo_centroidX +
            d3_geo_centroidY * d3_geo_centroidY +
            d3_geo_centroidZ * d3_geo_centroidZ
        ))
      ) > ε
    ) {
      return [
  ...
  function d3_geo_centroidPoint(λ, φ) {
    ...
    d3_geo_centroidX +=
      (cosφ * Math.cos(λ) - d3_geo_centroidX) / d3_geo_centroidW;
    d3_geo_centroidY +=
      (cosφ * Math.sin(λ) - d3_geo_centroidY) / d3_geo_centroidW;
  ...
  function d3_geo_clipSort(a, b) {
    return (
      ((a = a.point)[0] < 0 ? a[1] - π / 2 - ε : π / 2 - a[1]) -
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    projection.stream = function (stream) {
      return d3_geo_projectionRadiansRotate(
        rotate,
        preclip(projectResample(postclip(stream)))
      );
    };
    ...
    projection.clipExtent = function (_) {
      ...
      postclip =
        _ == null
          ? d3_identity
          : d3_geo_clipView(_[0][0], _[0][1], _[1][0], _[1][1]);
      return projection;
    };
  ...
  function d3_geo_circleInterpolate(radius, precision) {
    ...
    return function (from, to, direction, listener) {
      ...
      var point;
      for (
        var step = direction * precision, t = from;
        direction > 0 ? t > to : t < to;
        t -= step
      ) {
        listener.point(
  ...
  d3.geo.albersUsa = function () {
    ...
    albersUsa.invert = function (coordinates) {
      return (
        alaskaInvert(coordinates) ||
        hawaiiInvert(coordinates) ||
        puertoRicoInvert(coordinates) ||
    ...
    albersUsa.translate = function (x) {
      ...
      var dz = lower48.scale(),
        ...
        dy = x[1];
      lower48.translate(x);
      alaska.translate([dx - 0.4 * dz, dy + 0.17 * dz]);
      hawaii.translate([dx - 0.19 * dz, dy + 0.2 * dz]);
      puertoRico.translate([dx + 0.58 * dz, dy + 0.43 * dz]);
  ...
  function d3_geo_albersUsaInvert(projection, extent) {
    var a = projection(extent[0]),
      b = projection([0.5 * (extent[0][0] + extent[1][0]), extent[0][1]]),
    ...
    var cx =
        (0.5 *
          (ma * mb * (a[1] - c[1]) + mb * (a[0] + b[0]) - ma * (b[0] + c[0]))) /
        (mb - ma),
      cy = (0.5 * (a[0] + b[0]) - cx) / ma + 0.5 * (a[1] + b[1]);
    var dx0 = d[0] - cx,
      dy0 = d[1] - cy,
      dx1 = a[0] - cx,
      dy1 = a[1] - cy,
    ...
    return function (coordinates) {
      var dx = coordinates[0] - cx,
        ...
        r = dx * dx + dy * dy,
        a = Math.atan2(dy, dx);
      if (r0 < r && r < r1 && a0 < a && a < a1)
        return projection.invert(coordinates);
    };
  }
    ...
    function hull(data) {
      ...
      for (i = 1; i < n; ++i) {
        if (
          ...
          (vertices[i][1] == vertices[h][1] && vertices[i][0] < vertices[h][0])
        )
          h = i;
      }
      for (i = 0; i < n; ++i) {
  ...
  d3.scale.log = function () {
    return d3_scale_log(
      d3.scale.linear().domain([0, Math.LN10]),
      10,
      d3_scale_logp,
      d3_scale_powp,
      [1, 10]
  ...
  function d3_scale_log(linear, base, log, pow, domain) {
    ...
    scale.ticks = function () {
      ...
      if (extent.every(isFinite)) {
        ...
        if (log === d3_scale_logn) {
          ...
          for (; i < j; i++)
            for (var k = 1; k < n; k++) ticks.push(Math.pow(base, i) * k);
          ticks.push(Math.pow(base, i));
        }
        for (i = 0; ticks[i] < u; i++) {}
    ...
    scale.tickFormat = function (n, format) {
      ...
      var b = Math.log(base),
        k = Math.max(0.1, n / scale.ticks().length),
        f =
          log === d3_scale_logn
            ? ((e = -1e-12), Math.floor)
            : ((e = 1e-12), Math.ceil),
  ...
  d3_transitionPrototype.select = function (selector) {
    ...
    for (var j = -1, m = this.length; ++j < m; ) {
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if (
          (node = group[i]) &&
          (subnode = selector.call(node, node.__data__, i))
        ) {
  ...
  d3_transitionPrototype.delay = function (value) {
    ...
    return d3_selection_each(
      ...
      typeof value === 'function'
        ? function (node, i, j) {
            node.__transition__[id].delay =
              value.call(node, node.__data__, i, j) | 0;
          }
        : ((value |= 0),
          function (node) {
      ...
      d3.timer(
        function (elapsed) {
          var d = node.__data__,
            ...
            tweened = [];
          return (
            delay <= elapsed ? start(elapsed) : d3.timer(start, delay, time), 1
          );
  ...
  function d3_time_formatPad(value, fill, width) {
    ...
    return length < width
      ? new Array(width - length + 1).join(fill) + value
      : value;
  }
