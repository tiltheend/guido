...
WHERE PRODUCT_NO = 334;
 ...
 SELECT PRODUCT_NO, PRODUCT_NAME, PRODUCT_PACKAGE, PRODUCT_CONTENT, PRODUCT_PRICE,
	...
	(SELECT NVL(TO_CHAR(AVG(REVIEW_STARS), 'FM9.09'), '0.0') 
		FROM "REVIEW" 
		JOIN "PRODUCT" USING(PRODUCT_NO)
		WHERE PRODUCT_NO = 333) PRODUCT_RATING,
	(SELECT COUNT(*) FROM "REVIEW"
		WHERE PRODUCT_NO = 333) REVIEW_COUNT,
	(SELECT AVG(REVIEW_STARS)*20
		FROM "REVIEW" 
		JOIN "PRODUCT" USING(PRODUCT_NO)
		WHERE PRODUCT_NO = 333) REVIEW_PERCENTAGE,
	(SELECT COURSE_NAME FROM TOUR_COURSE 
		WHERE PRODUCT_NO = 333
...
UPDATE TOUR_COURSE 
SET BOSS_COURSE_FL = 'N'
WHERE COURSE_NAME = '다이소 경복궁역점'
AND
PRODUCT_NO =333;
...
DELETE FROM "PRODUCT"
WHERE PRODUCT_NAME LIKE '%24%';

DELETE FROM "FILE"
WHERE PRODUCT_NO IN (SELECT PRODUCT_NO FROM "PRODUCT" WHERE PRODUCT_NAME LIKE '%24%');

DELETE FROM "PRODUCT_OPTION"
WHERE PRODUCT_NO IN (SELECT PRODUCT_NO FROM "PRODUCT" WHERE PRODUCT_NAME LIKE '%24%');

DELETE FROM "PRODUCT_DT"
WHERE PRODUCT_NO IN (SELECT PRODUCT_NO FROM "PRODUCT" WHERE PRODUCT_NAME LIKE '%24%');
