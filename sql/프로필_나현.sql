...
SELECT * FROM (
  SELECT
    ...
    R.RESERVATION_NO, R.PRODUCT_NO, R.USER_NO, ORDER_NUMBER, R.TOTAL_PRICE,
    R.RESERVATION_STATE, R.PRODUCT_DT_NO, PO.OPTION_NAME,
    F.FILE_PATH, TO_CHAR(PD.PRODUCT_DT, 'YYYY-MM-DD') PRODUCT_DT,
    P.PRODUCT_NAME
  FROM
    RESERVATION R
    JOIN PRODUCT_DT PD ON PD.PRODUCT_DT_NO = R.PRODUCT_DT_NO
...
SELECT R.RESERVATION_NO, R.PRODUCT_NO, R.USER_NO, R.RESERVATION_STATE,
		  R.PRODUCT_DT_NO, R.ORDER_NUMBER, R.TOTAL_PRICE, PO.OPTION_NAME, P.PRODUCT_PACKAGE,
	...
	SELECT * FROM (
		  SELECT
		    ROW_NUMBER() OVER (ORDER BY PD.PRODUCT_DT DESC) AS NUM,
		      R.RESERVATION_NO, R.PRODUCT_NO, R.USER_NO, R.RESERVATION_STATE, P.TOUR_DURATION ,
			  R.PRODUCT_DT_NO, R.ORDER_NUMBER, R.TOTAL_PRICE, PO.OPTION_NAME, P.PRODUCT_PACKAGE,
			  F.FILE_PATH THUMBNAIL, TO_CHAR(PD.PRODUCT_DT, 'YYYY-MM-DD') PRODUCT_DT, P.PRODUCT_NAME
		  FROM RESERVATION R
		    JOIN PRODUCT_DT PD ON PD.PRODUCT_DT_NO = R.PRODUCT_DT_NO
...
SELECT PRODUCT_NO, PRODUCT_NAME, PRODUCT_PACKAGE, PRODUCT_STATE,
...
SELECT PRODUCT_NO, PRODUCT_PACKAGE, PRODUCT_PRICE, PRODUCT_ADD_PRICE, USER_NO
FROM PRODUCT
ORder BY 1;
...
FROM RESERVATION
JOIN PRODUCT USING (PRODUCT_NO)
WHERE PRODUCT.USER_NO= 23
AND IMP_UID IS NOT NULL
;

SELECT PRODUCT_NO, PRODUCT_NAME, PRODUCT_PACKAGE, USER_NO, TOUR_DURATION
FROM PRODUCT P
JOIN RESERVATION 
;
...
SELECT R.RESERVATION_NO, R.PRODUCT_NO, P.PRODUCT_NAME, P.PRODUCT_PACKAGE, R.USER_NO, P.TOUR_DURATION,
	        R.PRODUCT_DT_NO, PRODUCT_DT, R.OPTION_NO, OPTION_NAME,
	        CASE WHEN P.PRODUCT_PACKAGE >= 2 
	        	THEN PRODUCT_DT + (P.PRODUCT_PACKAGE - 1) AS PRODUCT_DT_END
	            ELSE PRODUCT_DT
...
SELECT R.RESERVATION_NO, R.PRODUCT_NO, P.PRODUCT_NAME, P.PRODUCT_PACKAGE, R.USER_NO, P.TOUR_DURATION,
    R.PRODUCT_DT_NO, PRODUCT_DT, R.OPTION_NO, OPTION_NAME,
    CASE
        WHEN P.PRODUCT_PACKAGE >= 2 THEN PRODUCT_DT + (P.PRODUCT_PACKAGE - 1)
        ELSE PRODUCT_DT
    END AS PRODUCT_DT_END
FROM RESERVATION R
RIGHT JOIN "PRODUCT" P ON (P.PRODUCT_NO = R.PRODUCT_NO)
...
SELECT REVIEW_NO, USER_NO, REVIEW_MESSAGE, REVIEW_STARS, REVIEW_DEL_FL, PRODUCT_NO, CREATE_DT, REVIEW_REPLY, REVIEW_REPLY_DEL_FL
FROM REVIEW;
...
UPDATE PR
SET BIRTH_YEAR=0, JOB='', PETS='', HOBBY='', SUB_LANG='', ABROAD_EXPERIENCE='', MBTI='', STRENGTH='', FAVORITE_SONG='', TMI='', MAJOR='', DOPAMINE='', USELESS_TALENT='', CAP_LIST=''
WHERE USER_NO=0
;
...
UPDATE "USER"
SET USER_PASSWORD='$2a$10$YrdxePV.fKErBDAGWzRpKuB950OErdL.7IZiMI.O/KuNv1Qume7Me';
